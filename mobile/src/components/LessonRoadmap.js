      ...
      {isCurrent && (
        <Animated.View
          style={[
            styles.glowRing,
            {
              left: cx - (NODE_SIZE + 28) / 2,
              top: cy - (NODE_SIZE + 28) / 2,
      ...
      {isCurrent && (
        <View
          style={[
            styles.jumpPillWrap,
            pillOnLeft
              ? { right: CARD_W - (cx - NODE_SIZE / 2 + 6), top: cy + 6 }
              : { left: cx + NODE_SIZE / 2 - 2, top: cy + 6 },
          ]}
        >
          <PuckButton
            color="#FFFFFF"
          ...
          >
            <Text style={[styles.jumpPillText, { color: shadeHex(theme.color, -0.12) }]}>JUMP HERE?</Text>
          </PuckButton>
        </View>
      )}
    </>
...
function ModuleSection({
  ...
  collapsed, onToggle, onLessonPress, pulse, onLayout,
}) {
    ...
    <View style={styles.section} onLayout={onLayout}>
      ...
      >
        <View style={{ flex: 1 }}>
          ...
          <Text style={styles.sectionTitle} numberOfLines={2}>{module.title}</Text>
        ...
        <PuckButton
          ...
          onPress={onToggle}
        >
          <Ionicons name={collapsed ? 'chevron-down' : 'chevron-up'} size={20} color={ROADMAP_GREEN.solid} />
        </PuckButton>
      </PuckButton>
      ...
      {modules.map((module, i) => {
        ...
        return (
          <ModuleSection
            ...
            onToggle={() => onToggle(module.id, isCollapsed)}
            onLessonPress={onLessonPress}
            pulse={pulse}
            onLayout={(e) => onSectionLayout?.(module.id, e.nativeEvent.layout.y)}
          />
        );
      })}
    </>
...
const makeStyles = (colors) => StyleSheet.create({
  ...
  sectionTitle: { fontSize: 20, fontWeight: '800', color: '#FFFFFF', marginBottom: 10, letterSpacing: -0.4 },
